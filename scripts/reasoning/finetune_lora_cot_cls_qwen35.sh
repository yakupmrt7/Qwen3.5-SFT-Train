#!/usr/bin/env bash
#SBATCH --job-name=qwen35-cot-cls
...
#SBATCH --output=/arf/scratch/aalatan/Re-CoT/Qwen-VL-Series-Finetune/output/qwen35-cot-cls-%j.out
#SBATCH --error=/arf/scratch/aalatan/Re-CoT/Qwen-VL-Series-Finetune/output/qwen35-cot-cls-%j.err

# Stage 2: chain-of-thought training on top of the merged CLS SFT model.
# A fresh LoRA adapter is trained over qwen35-cls-merged (base weights frozen).
...
MODEL=$REPO/output/qwen35-cls-merged                                        # stage-1 SFT model, LoRA already merged in
DATA=/arf/scratch/aalatan/Re-CoT/datasets/reasoning/CLS/cls_sft_reasoning.json
IMAGE_FOLDER=/arf/scratch/aalatan/Re-CoT/datasets/reasoning/CLS/images      # image field is "<DATASET>/<name>", resolved under here
OUTPUT_DIR=$REPO/output/qwen35-cot-cls-lora
